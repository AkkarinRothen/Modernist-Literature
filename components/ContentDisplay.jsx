...
import { type ModernistInfo, type AuthorDetails, type ComparativeAnalysis } from '../types.js';
import { fetchAuthorBiography } from '../services/geminiService.js';
import CharacterRelationshipDiagram from './CharacterRelationshipDiagram.jsx';
import ComparisonDisplay from './ComparisonDisplay.jsx';
...
const LoadingIndicator: React.FC<{ topicName: string | null, comparisonTopics: string[] }> = ({ topicName, comparisonTopics }) => {
    ...
    return (
        <div role="status" aria-live="polite" className="flex flex-col items-center justify-center h-full text-center">
            <div className="w-full max-w-lg">
                <svg aria-hidden="true" className="animate-spin h-8 w-8 text-neutral-500 mx-auto mb-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                <h2 className="text-2xl sm:text-3xl font-semibold text-neutral-700 tracking-tight">
                    {title}
                </h2>
...
const InitialState: React.FC = () => (
    <div className="flex flex-col items-center justify-center h-full text-center text-neutral-500">
        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mb-4 text-neutral-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
        ...
        <h2 className="text-3xl sm:text-4xl font-semibold text-neutral-700">Explore Modernism</h2>
...
const ComparisonInitialState: React.FC<{ selectionCount: number }> = ({ selectionCount }) => (
    <div className="flex flex-col items-center justify-center h-full text-center text-neutral-500">
         <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mb-4 text-blue-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
        <h2 className="text-3xl sm:text-4xl font-semibold text-neutral-700">Comparative Analysis</h2>
        <p className="mt-2 max-w-lg">
            {selectionCount < 2 
...
const ErrorDisplay: React.FC<{ message: string; onRetry: () => void }> = ({ message, onRetry }) => (
    <div role="alert" className="flex flex-col items-center justify-center h-full text-center text-red-700 bg-red-50 p-8 rounded-lg">
        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <button
            onClick={onRetry}
            className="mt-6 px-6 py-2 bg-red-600 text-white font-semibold rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors"
        >
            Try Again
        </button>
    </div>
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in space-y-12">
      <header className="pb-4 border-b border-neutral-200">
        <h2 className="text-4xl sm:text-5xl font-bold text-neutral-900 tracking-tighter">{content.topic}</h2>
      </header>
      
      <section>
        <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-4">Summary</h3>
        <p className="text-base sm:text-lg text-neutral-600 leading-relaxed whitespace-pre-wrap">{content.summary}</p>
      </section>

      {content.setting && (
        <section>
          <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-4">Setting</h3>
          <p className="text-base sm:text-lg text-neutral-600 leading-relaxed">{content.setting}</p>
        </section>
      )}

      {content.characters && content.characters.length > 0 && (
        <section>
          <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-4">Key Characters</h3>
          <div className="space-y-4">
            {content.characters.map((char, index) => (
              <div key={index} className="bg-neutral-50/70 p-4 rounded-lg border border-neutral-200">
                <h4 className="font-bold text-base sm:text-lg text-neutral-900">{char.name}</h4>
      ...
      {content.characterRelationships && content.characterRelationships.length > 0 && (
        <section>
          <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-4">Character Relationships</h3>
      ...
      {content.themes && content.themes.length > 0 && (
          <section>
            <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-6">Major Themes</h3>
      ...
      {content.symbols && content.symbols.length > 0 && (
        <section>
          <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-4">Symbolism</h3>
          <dl className="space-y-4">
            {content.symbols.map((item, index) => (
              <div key={index} className="bg-neutral-50/70 border-l-4 border-neutral-800 pl-4 py-3">
                <dt className="font-bold text-base sm:text-lg text-neutral-900">{item.name}</dt>
      ...
      {content.characteristics && content.characteristics.length > 0 && (
        <section>
          <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-6">Key Characteristics</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <section>
        <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-4">Works Cited</h3>
      ...
      {authorBios && Object.keys(authorBios).length > 0 && (
        <section>
          <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-4">Author Biographies</h3>
          <div className="space-y-4">
              ...
              <div key={author} className="flex items-start bg-neutral-50/70 p-4 rounded-lg border border-neutral-200 space-x-4">
                ...
                <div>
                  <div className="flex justify-between items-baseline">
                    <h4 className="font-bold text-base sm:text-lg text-neutral-900">{author}</h4>
      ...
      {content.relatedConcepts && content.relatedConcepts.length > 0 && (
        <section>
          <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-4">Related Concepts</h3>
      ...
      {content.glossary && content.glossary.length > 0 && (
        <section>
          <h3 className="text-2xl sm:text-3xl font-semibold text-neutral-800 mb-4">Glossary of Terms</h3>
          <dl className="space-y-4">
            {content.glossary.map((item, index) => (
              <div key={index} className="bg-neutral-50/70 border-l-4 border-neutral-800 pl-4 py-3">
                <dt className="font-bold text-base sm:text-lg text-neutral-900">{item.term}</dt>
